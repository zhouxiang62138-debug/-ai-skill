...
`INTAKE → Research Gate → optional REQUIREMENT_RESEARCH → INTAKE → optional REFERENCE_ANALYSIS → PLANNING → DESIGN_EXPLORATION → WAITING_FOR_DESIGN_REVIEW → PLANNING_REVISION → WAITING_FOR_PRODUCT_REVIEW → WAITING_FOR_PLAN_REVIEW → APPROVED_FOR_IMPLEMENTATION → IMPLEMENTING`

其中 `REQUIREMENT_RESEARCH` 与 `REFERENCE_ANALYSIS` 是可选 Module 路由，不是
第四个 Agent。Research Gate 不能被跳过；Research Execution 可被确定性判定为
`required`、`optional` 或 `not_required`。只有需要执行时才进入 `REQUIREMENT_RESEARCH`
并回到 Intake Gate；没有研究需求时不执行外部 Research。Reference Analysis 完成后进入
Planner。
